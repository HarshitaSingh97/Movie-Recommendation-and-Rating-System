...

{
  ...
  $movie_id[]=$row[0];
	...
	$avguserret="";
	$urlret="";
$query1 = "select m.title from movie m where m.release_date >= DATE_FORMAT(CURDATE(), '%Y-%m-01') - INTERVAL 1 MONTH";
$query2 = "select m.average_vote from movie m where m.release_date >= DATE_FORMAT(CURDATE(), '%Y-%m-01') - INTERVAL 1 MONTH";
$query3 = "select m.url from movie m where m.release_date >= DATE_FORMAT(CURDATE(), '%Y-%m-01') - INTERVAL 1 MONTH";
$query4 = "select avg(uw.user_rating) from movie m LEFT JOIN user_watched uw on m.movie_id=uw.movie_id where m.release_date >= DATE_FORMAT(CURDATE(), '%Y-%m-01') - INTERVAL 1 MONTH group by m.movie_id;";
