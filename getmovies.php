...
$username=$_SESSION['username'];
	...
	if(count($genres)==1 and count($languages)==1){
		$query1 = "select title from movie where title like '$search';";
		$query2 = "select average_vote from movie where title like '$search';";
		$query3 = "select url from movie where title like '$search';";
		$query4 = "select avg(uw.user_rating) from movie m LEFT JOIN user_watched uw on m.movie_id=uw.movie_id where m.title like '$search' group by m.movie_id;";
		...
		for($i=0; $i < count($genres)-1;$i++)
		{
			for($j=0;$j < count($languages)-1;$j++)
			{
				...
				$query4 = "select avg(uw.user_rating) from movie m LEFT JOIN user_watched uw on m.movie_id=uw.movie_id where m.title like '$search' and language='$languages[$i]' and m.movie_id in (select mg.movie_id from genre g, movie_genres mg where g.genre_id=mg.genre_id and g.genre_name='$genres[$i]') group by m.movie_id;";
				processquery($query1,$query2,$query3,$query4,$db_server);
			}
		}
	}
