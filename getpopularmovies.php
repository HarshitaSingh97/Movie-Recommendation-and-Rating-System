...

{
  ...
  $movie_id[]=$row[0];
	...
	$avguserret="";
	$urlret="";
$query1 = "select m.title from movie m order by m.average_vote DESC";
$query2 = "select m.average_vote from movie m order by m.average_vote DESC";
$query3 = "select m.url from movie m order by m.average_vote DESC";
$query4 = "select avg(uw.user_rating) from movie m LEFT JOIN user_watched uw on m.movie_id=uw.movie_id group by m.movie_id order by m.average_vote DESC;";
